...
impl_runtime_apis! {
	...
	impl frontier_rpc_primitives::EthereumRuntimeApi<Block> for Runtime {
		...
		fn block_by_number(number: u32) -> (
			Option<EthereumBlock>, Vec<Option<pallet_ethereum::TransactionStatus>>
		) {
			if let Some(block) = <pallet_ethereum::Module<Runtime>>::block_by_number(number) {
				let statuses = <pallet_ethereum::Module<Runtime>>::block_transaction_statuses(&block);
				return (
					Some(block),
					statuses
				);
		...
		fn block_transaction_count_by_hash(hash: H256) -> Option<U256> {
			if let Some(block) = <pallet_ethereum::Module<Runtime>>::block_by_hash(hash) {
				return Some(U256::from(block.transactions.len()))
			}
			None
		}
		...
		fn block_by_hash(hash: H256) -> Option<EthereumBlock> {
			<pallet_ethereum::Module<Runtime>>::block_by_hash(hash)
		}
		...
		fn transaction_by_block_hash_and_index(hash: H256, index: u32) -> Option<(
			...
			EthereumBlock,
			TransactionStatus)> {
			<pallet_ethereum::Module<Runtime>>::transaction_by_block_hash_and_index(hash, index)
		}
		...
		fn transaction_by_block_number_and_index(number: u32, index: u32) -> Option<(
			...
			EthereumBlock,
			TransactionStatus)> {
			<pallet_ethereum::Module<Runtime>>::transaction_by_block_number_and_index(
				number,
				index
			)
		}
